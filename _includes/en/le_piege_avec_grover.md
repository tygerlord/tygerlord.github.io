# The trap with Grover

During my wanderings on the net, I came across the blog of 'Balise' describing [the SAT problem](https://blog.pasithee.fr/2013/02/25/le-probleme-sat/) and as I was also fiddling with quantum at that time, I set out to solve the example given in the Balise blog by a quantum method.

<br>

So I started by reproducing the example function composed of logic gates with quantum gates, I put in the oven (200 ° C, 1ms) and impecably reproduced the expected truth table. Of course, no quantum acceleration with this process. So I wanted to go further, and touch this miraculous computing power promised using [Grover]'s algorithm (https://en.wikipedia.org/wiki/Grover%27s_algorithm), And there patatrac, I came across a bone, it did not work. Obviously I could only have made a mistake in my implementation, I refuse to believe that this computing power has a problem, the grail can not have a small defect. So I tortured the circuits in all directions, nothing to do.

<br>

It is with a heavy heart that I resolved to tell myself that the Grover algorithm did not work in this case, which called into question its use, indeed difficult to imagine using an algorithm that can tell you that there are no solutions when on the contrary the problem has several. Fortunately after reflection, I found that there were simple ways to get around the problem and thus have the certainty of proper operation in all cases.  

<br>

Before detailing my misadventure, I advise you for those who want to approach this world full of promise of quantum to visit [Quantum: A little mathematics for quantum computing](https://exo7math.github.io/quantum-exo7/), a great resource on the subject as well as the excellent blog of [Vivien Londe](https://vivienlonde.github.io/blog) and of course the tools [Qiskit](https://qiskit.org/textbook/ch-algorithms/grover.html)

<br>

Let's go to the details that you can find in full on [github](https://github.com/tygerlord/le-probleme-sat/blob/main/le_piege_avec_grover.ipynb)
...
# Execution, Results and Visualization

Truth table of F, expected result.
...
The 4th bit indicates the result of F for the values of the first 3 bits

ex)
0101 F=1 for x=0, y=1, z=0
...
We find our 4 solutions from the previous table On the other hand question performance, as it is, we gain nothing at all level complexity of the algorithm, worse, we degrade since experimentally it takes at least 16 loops while, in classic version, there are 8 combinations to test. There is a dimension in our circuit that we did not use, probabilities.

# Use of quantum properties

To go further, it is necessary to succeed in using quantum properties to orient the solution(s) towards a higher probability using the [Grover](https://en.wikipedia.org/wiki/Grover%27s_algorithm] algorithm.


# Construction of circuit 2

Using Grover's algorithm. The documentation of [Qiskit](https://qiskit.org/textbook/ch-algorithms/grover.html) provides an example and complete explanations of this algorithm.
...
# Execution, Results and Visualization 2
...
The result is rather disappointing! These values do not make it possible to determine a solution to our function F when we know that there are four... 

To try to understand what is happening, let's develop the algorithm on 3 functions having respectively 3, 4 and 0 solutions.
...
# Execution, Results and Visualization (x+z).!y
...
```
    ...
    Number of '0' and '1': {'010': 35, '100': 286, '110': 33, '001': 288, '101': 285, '000': 41, '111': 26, '011': 30}
...
The result is in line with our expectations, we find our 3 solutions!

# Circuit construction (x+z).(!x+!y)
...
# Execution, Results and Visualization (x+z).(!x+!y)
...
There, something is happening that is wrong, we do not get our 4 solutions.

# Circuit construction (y).!y
...
# Execution, Results and Visualization (y).!y
...
Here it is rather in line with what we are entitled to expect, there are no possible solutions.



Grover's algorithm does not seem to work properly when you have a lot of solutions, let's say at least N/2 solutions, you get a result substantially identical to 0 solutions. (I leave it to the experienced mathematicians to justify or not this behavior of the algorithm)

How to avoid this pitfall?

If possible, determine in advance the number of solutions and check that it is less than N/2.

If we can not determine the number of solutions, I considered 2 possibilities according to the use cases*

1) We can, if we come across a zero solution case, randomly test some solutions, if we are in a case of many solutions we have a good chance of finding one.

2) We can also add an additional qubit of input and integrate it into the Oracle, which will artificially increase the number of combinations and therefore drop the number of solutions under (N+1)/2.
...
# Execution, Results and Visualization 3
...
We find the result of our function F:
...
Despite this excellent result, all this lacks a mathematical justification. In addition in theory we would have to √ (N / M) rotations with 4 qubits N = 16 and 4 solutions M = 4 it is 2 rotations or the result is very good in one rotation. It will therefore be necessary to study more closely what exactly is happening as well as the impact of this additional qubit on other cases.
